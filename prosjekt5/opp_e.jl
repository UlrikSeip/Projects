...
function main(a,b,c,f,S0,I0,R0,T,dt,filename)
    """
    ...
    for j = 1:length(t)-1
        ...
        s = RungeKutta4(dt, dSf, S[j], t[j], [c,R[j],a,I[j],N,f])
        i = RungeKutta4(dt, dIf, I[j], t[j], [a,S[j],N,b])
        r = RungeKutta4(dt, dRf, R[j], t[j], [c,I[j],b,S[j],f])
        push!(S,s)
        push!(I,i)
        push!(R,r)
    end
...
main(4,1,0.5, 1, 300,100,0, 13,.01, "opp_a_A.pdf")
